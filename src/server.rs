...
use tree_sitter::{self as tree_sitter_rt, Node, Parser, Query, QueryCursor, Tree};
...
pub struct Backend {
    pub client: Client,
    pub files: Arc<RwLock<HashMap<Url, FileData>>>,
...
impl LanguageServer for Backend {
    ...
    async fn hover(&self, params: HoverParams) -> Result<Option<Hover>> {
        ...
        let Some(node) = root.named_descendant_for_point_range(
            tree_sitter_rt::Point::new(position.line as usize, position.character as usize),
            tree_sitter_rt::Point::new(position.line as usize, position.character as usize + 1),
        ) else {
            return Ok(None);
        };
...
impl Backend {
    ...
    pub async fn update_content(&self, uri: Url, mut text: String) {
    ...
    pub async fn update_definitions(&self, uri: &Url, diagnostics: &mut Vec<Diagnostic>) {
        let mut files = self.files.write().await;
    ...
    pub async fn check_types(&self, uri: &Url, diagnostics: &mut Vec<Diagnostic>) {
        let files = self.files.read().await;
        let Some(file_data) = files.get(uri) else {
            return;
        };
    ...
    pub async fn run_diagnostics(&self, uri: &Url) {
        ...
        // Overlength checks
        {
            ...
            cursor.set_point_range(
                tree_sitter_rt::Point::new(config.max_lines, 0)
                    ..tree_sitter_rt::Point::new(usize::MAX, usize::MAX),
            );
            let query = Query::new(tree_sitter_ic10::language(), "(instruction)@x").unwrap();
...
pub fn get_current_parameter(instruction_node: Node, position: usize) -> (usize, Option<Node>) {
    let mut ret: usize = 0;
...
impl From<tree_sitter_rt::Point> for Position {
    fn from(value: tree_sitter_rt::Point) -> Self {
...
impl From<Position> for tree_sitter_rt::Point {
    fn from(value: Position) -> Self {
        tree_sitter_rt::Point {
...
impl From<tree_sitter_rt::Range> for Range {
    fn from(value: tree_sitter_rt::Range) -> Self {
